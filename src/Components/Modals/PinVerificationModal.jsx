...
const PinVerificationModal = ({
  ...
  if (pinRefs.current.length !== 4) {
    pinRefs.current = Array(4)
      .fill()
      .map((_, i) => pinRefs.current[i] || React.createRef());
  }
  ...
  const getTransactionSummary = () => {
    ...
    // ── Scheduled payments (single / P2P / bulk) ──────────────────────────────
    if (transactionType === "scheduled-payment") {
      const isBulk = Array.isArray(transactionData.items);
      ...
      if (isBulk) {
        ...
        return {
          title: "Scheduled Bulk Payout",
          recipient: `${transactionData.items.length} recipient${
            transactionData.items.length !== 1 ? "s" : ""
          }`,
          amount: totalAmount,
          bank: "Multiple Banks",
          extra: [
            {
              ...
              value: transactionData.groupKey || "—",
            },
            {
              label: "Schedule Type",
              value: transactionData.scheduleType || "—",
            },
            ...
            {
              label: "Starts",
              value: transactionData.startAt
                ? new Date(transactionData.startAt).toLocaleString("en-NG", {
                    day: "numeric",
                    month: "short",
                    year: "numeric",
                    hour: "2-digit",
      ...
      // Single / P2P scheduled payment
      const isP2P = !!transactionData.destinationTagOrCode;
      return {
        title: isP2P ? "Scheduled P2P Transfer" : "Scheduled Bank Transfer",
        recipient: isP2P
          ? transactionData.destinationTagOrCode
          : transactionData.beneficiaryId || "—",
        amount: transactionData.amount,
        bank: isP2P ? "P2P / Tag Transfer" : "—",
        extra: [
          {
            label: "Schedule Type",
            value: transactionData.scheduleType || "—",
    ...
    switch (transactionType) {
      ...
      default:
        return {
          title: "Bank Transfer",
          recipient:
            transactionData.beneficiaryName || transactionData.accountNumber,
          amount: transactionData.amount,
